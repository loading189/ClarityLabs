...
export type PlanFromActionResponse = {
  plan_id: string;
  created: boolean;
};

export function createPlanFromAction(businessId: string, actionId: string) {
  return apiPost<PlanFromActionResponse>(`/api/plans/${encodeURIComponent(businessId)}/from_action`, {
    action_id: actionId,
  });
}
