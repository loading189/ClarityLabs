...
from backend.app.signals.v2_anchors import _add_ledger_anchor, _anchor_query
...
from backend.app.signals.v2_explain import _explanation_payload
...
def detect_expense_creep_by_vendor(
    ...
    *,
    ...
    for vendor_key, current_total in current.items():
        ...
        current_vendor_txns = current_txns.get(vendor_key) or []
        prior_vendor_txns = prior_txns.get(vendor_key) or []
        evidence_txns = current_vendor_txns + prior_vendor_txns
        ...
        current_rows = _evidence_source_event_ids(current_vendor_txns)
        prior_rows = _evidence_source_event_ids(prior_vendor_txns)
        payload = {
...
def detect_low_cash_runway(
    ...
    *,
    ...
    _add_ledger_anchor(
        ...
        _anchor_query(
            start_date=burn_start.isoformat(),
            end_date=burn_end.isoformat(),
...
def detect_liquidity_cash_trend_down(
    ...
    *,
    ...
    current_avg, current_days = _avg_balance(
        [balance_by_date[d] for d in balance_by_date if current_start <= d <= current_end]
    )
    prior_avg, prior_days = _avg_balance(
        [balance_by_date[d] for d in balance_by_date if prior_start <= d <= prior_end]
    )
    ...
    _add_ledger_anchor(
        ...
        _anchor_query(
            ...
            source_event_ids=current_txn_ids,
        ),
    ...
    *,
    ...
    _add_ledger_anchor(
        ...
        _anchor_query(
            start_date=current_start.isoformat(),
            end_date=current_end.isoformat(),
            source_event_ids=inflow_txn_ids,
            direction="inflow",
        ),
        evidence_keys=["current_total"],
    )
    ...
    *,
    ...
    _add_ledger_anchor(
        ...
        _anchor_query(
            start_date=current_start.isoformat(),
            end_date=current_end.isoformat(),
            source_event_ids=outflow_txn_ids,
            direction="outflow",
        ),
        evidence_keys=["current_total"],
    )
    ...
    for vendor_key in sorted(current_by_vendor.keys()):
        ...
        _add_ledger_anchor(
            ...
            _anchor_query(
                ...
                end_date=current_end.isoformat(),
                vendors=[vendor_name],
                source_event_ids=vendor_txn_ids,
                direction="outflow",
            ),
...
def detect_timing_payroll_rent_cliff(
    ...
    *,
    ...
    _add_ledger_anchor(
        ...
        _anchor_query(
            start_date=cliff_date.isoformat(),
            end_date=cliff_date.isoformat(),
            source_event_ids=cliff_txn_ids,
            direction="outflow",
        ),
    ...
    *,
    ...
    _add_ledger_anchor(
        ...
        _anchor_query(
            ...
            source_event_ids=uncategorized_txn_ids,
        ),
