...
def _intervention_library() -> List[InterventionTemplate]:
    return [
        InterventionTemplate(
            kind="revenue_drop",
            label="Revenue drop",
            description="Sustained drop in demand (volume or ticket).",
            defaults={"pct": 0.30, "mode": "volume"},  # mode: "volume" | "ticket"
            fields=[
                InterventionTemplateField(key="pct", label="Drop (%)", type="percent", default=0.30, min=0.05, max=0.95, step=0.01),
                InterventionTemplateField(key="mode", label="Affects", type="text", default="volume"),
            ],
        ),
        InterventionTemplate(
            kind="expense_spike",
            ...
            fields=[
                InterventionTemplateField(key="pct", label="Increase (%)", type="percent", default=0.25, min=0.05, max=5.0, step=0.01),
                InterventionTemplateField(key="category", label="Category", type="text", default="cogs"),
            ],
        ),
        InterventionTemplate(
            kind="deposit_delay",
            ...
            fields=[
                InterventionTemplateField(key="days", label="Days delayed", type="days", default=3, min=1, max=14, step=1),
                InterventionTemplateField(key="pct_affected", label="Share affected (%)", type="percent", default=0.8, min=0.1, max=1.0, step=0.05),
            ],
        ),
        InterventionTemplate(
            kind="refund_spike",
            ...
            defaults={"refund_rate": 0.08},  # probability per order to produce a refund event
            fields=[
                InterventionTemplateField(key="refund_rate", label="Refund rate (%)", type="percent", default=0.08, min=0.0, max=0.5, step=0.01),
            ],
        ),
        InterventionTemplate(
            ...
            defaults={"pct": 0.15},
            fields=[
                InterventionTemplateField(key="pct", label="Increase (%)", type="percent", default=0.15, min=0.01, max=1.0, step=0.01),
            ],
        ),
    ]
...
def _to_out(cfg: SimulatorConfig) -> SimulatorConfigOut:
    return SimulatorConfigOut(
...
def list_sim_interventions(business_id: str, db: Session = Depends(get_db)):
    ...
    if not isinstance(ivs, list):
        ...
        db.add(prof)
        db.commit()
    return [InterventionOut(**iv) for iv in ivs if isinstance(iv, dict)]
...
def run_history_legacy(business_id: str, req: Dict[str, Any], db: Session = Depends(get_db)):
    ...
    """
    start = (utcnow() - timedelta(days=int(req.get("days", 120)))).date().isoformat()
    gen = GenerateIn(
        start_date=start,
    ...
    return generate_history(business_id, gen, db)  # type: ignore[arg-type]
