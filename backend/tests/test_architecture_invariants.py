...
from backend.app.services import monitoring_service, signals_service, sim_service
...
def test_sim_pulse_delegates_to_monitoring_service() -> None:
    route_source = inspect.getsource(sim_routes.pulse)
    service_source = inspect.getsource(sim_service.pulse)

    assert "sim_service.pulse" in route_source
    assert "monitoring_service.pulse" in service_source


def test_api_signals_list_uses_state_store_only() -> None:
    list_source = inspect.getsource(signals_routes.list_signals)

    assert "list_signal_states" in list_source
    assert "fetch_signals" not in list_source
